...
- [X] T004 Implement `levenshteinDistance` and `fuzzyMatchTask` in `src/app/utils/search.util.ts`
- [X] T005 Add unit tests for `levenshteinDistance` in `src/app/utils/search.util.spec.ts`
...
- [X] T009 [US1] Update `src/app/features/kanban/board/board.component.ts` to filter tasks using `fuzzyMatchTask`
